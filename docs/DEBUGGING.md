...
CH32V203 当前配方生成独立 `wch.ch32v203/0.1.1` 包，收录 11 个型号，按容量和 SDK 分支区分配置，V203 寄存器窗口排除 FPU。原有 11 个裸机型号通过编译和离线检查；C8T6 + WCH-LinkE 在用户授权解除读保护后通过 IDE 基础调试实板验收。新版为 V20x 分支十个型号增加 FreeRTOS，CCT6 继续保留裸机模板；新增 RTOS 模板不沿用此前裸机实板验收。

CH32V307VCT6 / RCT6 / WCU6 当前配方生成 `wch.ch32v307/0.1.3` 器件包，使用 `wch.riscv/1.0.0` 工具集和 WCH-Link / WCH-LinkE，工作在 RISC-V / SDI 模式。当前只接受 256 KiB Flash / 64 KiB RAM 布局、400/4000/6000 kHz 和单台探针（序列号留空）。不支持把通用 RISC-V、ST-Link 或 CMSIS-DAP 配置套用到 CH32。现有 0.1.1 裸机工程仍可用；0.1.0 工程没有下载/调试配置，不自动升级工程。VCT6 的隔离 FreeRTOS 验收工程已通过实板 MCP 调试；RCT6/WCU6 尚未完成 RTOS 实板验收，见 [FreeRTOS 验证范围](RTOS_DEBUGGING.md#已完成的实板观测)。
...
0.2.4 中下载与调试附加共用实际校验输出检查：必须收到 `verified N bytes` 且 N 与当前映像完整可加载字节数一致，并拒绝实际字节差异、USB 失败或输出截断。WCH OpenOCD 即使在逐字节差异后返回 MI `done`，也不能算匹配；CRC 警告后的完整逐字节回退通过且无差异时可以接受。校验失败保留原始工具诊断，调试不会自动烧录以消除差异。
...
3. 点击「调试」或 Ctrl+F5，连接成功后使用断点、运行、暂停、单步、调用栈、变量、只读内存、反汇编与 RTOS 窗口。初次连接停在板上程序当前执行位置；如需从头执行，设置 main 内断点，再点「复位」「运行」。
...
- 底部：调试页签只在调试会话中显示；结束时若仍选中调试页，则切回构建页。其中调用栈与变量观察并排，另有局部变量、断点管理、只读内存、反汇编、RTOS、断点日志和 MI 原始输出页签。
...
当前未提供寄存器写入、任意表达式执行和外设 SVD。变量观察支持输入符号、成员路径、固定下标、寄存器名；离线模型只解析示例中的符号，其他项显示不可用。

## 反汇编

在底部「调试 → 反汇编」查看地址、机器码、指令与函数内偏移，黄色 `▶` 标出暂停时的真实 PC。默认从 PC 向后读取 128 字节；启用「跟随暂停位置」后，暂停或单步时自动刷新。「选中栈帧」可浏览调用者地址，「当前 PC」回到执行位置；调用者返回地址不会冒充当前 PC。支持手动输入地址和前后浏览。

读取仅在暂停状态允许。运行中保留淡化的过时快照并取消 PC 标记；结束、断开或切换工程后清空，迟到结果不能覆盖新会话。F407 离线示例中的指令明确标为模拟，不能当作工程 ELF 或实机 Flash 数据。

内置 Agent 和外部 MCP 使用相同的只读 `debug_disassemble`，地址省略时读取当前 PC。后端使用 GDB/MI `-data-disassemble`；单次范围 1–512 字节，默认 128，结束地址为排他边界。范围限制用于单次响应，不限制会话调用次数。

## FreeRTOS 内核状态

暂停目标后，在底部「调试 → RTOS」或菜单「调试 → FreeRTOS 状态」查看任务、调度器、堆及队列/信号量/互斥量。内置 Agent 与外部 MCP 共用只读 `debug_rtos_snapshot`。通过现有会话的 ELF 类型、符号和目标内存读取，不调用内核函数或写入目标；缺字段时显示空值和诊断。使用配置、句柄观察、快照生命周期与两块板的实测范围见 [FreeRTOS 调试](RTOS_DEBUGGING.md)。
